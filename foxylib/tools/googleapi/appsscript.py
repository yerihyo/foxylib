from foxylib.tools.googleapi.utils import username_scope2creds, scope_str2url,\
    username2filepath_credentials_json, username_scope2filepath_token_json
...
#             response = service.projects().get(scriptId=script_id).execute()
    ...
    def test_RAW(cls):
        ...
        filepath_token_json = username_scope2filepath_token_json(username_GOOGLE, cls.SCOPE_PROJECT)
    
        store = file.Storage(filepath_token_json)
        creds = store.get()
        if not creds or creds.invalid:
            flow = client.flow_from_clientsecrets(filepath_credentials_json, scope_str2url(cls.SCOPE_PROJECT))
